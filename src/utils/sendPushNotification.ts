...
export async function sendPushNotification(userId: string, title: string, body: string, dbData?: object)  {
    try {
        const userRef = firestore.collection("fcmTokens").doc(userId);
        const doc = await userRef.get();

        if (!doc.exists) return console.log("No FCM tokens found for user:", userId);
        ...
        response.responses.forEach((res, index) => {
            if (!res.success && (res.error?.code === "messaging/registration-token-not-registered" || res.error?.code === "messaging/invalid-argument")) {
